/*global history */
sap.ui.define([
	"bootcamp/controller/BaseController",
	"sap/ui/model/json/JSONModel",
	"sap/ui/model/Filter",
	"sap/ui/model/FilterOperator",
	"sap/m/GroupHeaderListItem",
	"sap/ui/Device",
	"bootcamp/model/formatter"
], function (BaseController, JSONModel, Filter, FilterOperator, GroupHeaderListItem, Device, formatter) {
	"use strict";
	return BaseController.extend("bootcamp.controller.Master", {
		formatter: formatter,
		/* =========================================================== */
		/* lifecycle methods                                           */
		/* =========================================================== */
...
_showDetail: function (oItem) {
			...
			this.getRouter().navTo("object", { objectId: oItem.getBindingContext().getProperty("JOBID") }, bReplace);
		},
		/**
...
_updateFilterBar: function (sFilterBarText) {
		...
		/**
	*@memberOf bootcamp.controller.Master
	*/
addJob: function () {
			...
			var url = this.getView().getModel().sServiceUrl.substr(0,35)+"/update.xsjs?$mode=ins";
			
			/*$.post(this.getView().getModel().sServiceUrl.substr(0,35)+"/update.xsjs?$mode=ins",JSON.stringify(oJsonData),function(){
				alert("Update Success!");
				url:this.getView().getModel().sServiceUrl.substr(0,35)+"/update.xsjs?$mode=ins",
				url:"https://bootcamphanas0015902459trial.hanatrial.ondemand.com/bootcamp//update.xsjs?$mode=ins",
			});*/
			$.ajax({
				url:url,
				type:"GET",
				...
				success: function(reqdata, textStatus, XMLHttpRequest) {
                    var token = XMLHttpRequest.getResponseHeader('X-CSRF-Token');
                    var data = JSON.stringify(oJsonData);
                    $.ajax({
                        url: url,
                        type: "POST",
                        ...
                        error: function() {alert("works not");}
                    });		
                }
			});
		}
